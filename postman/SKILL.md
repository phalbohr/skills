...
4. Only use `mcp__postman-official__*` tools directly for single targeted updates or when the user explicitly asks.
...
This skill uses `mcp__postman-official__*` (the official `@postman/postman-mcp-server` npm package).

**Tools NOT available in this server** (referenced in older docs):
- `searchPostmanElements` — not available; use `getCollections` with workspace ID instead
- `getCollectionRequest` / `getCollectionResponse` / `getCollectionFolder` — not available; use `getCollection` for full detail
- `createCollectionFolder` — not available
- `updateCollectionResponse` — not available
- `publishDocumentation` / `unpublishDocumentation` — not available
- `getCollectionUpdatesTasks` — not available; use `getGeneratedCollectionSpecs` or `getSpecCollections` for async polling
- `getCodeGenerationInstructions` — not available

When a workflow step references an unavailable tool, skip it or use the nearest available alternative.
